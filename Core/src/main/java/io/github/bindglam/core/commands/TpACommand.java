...
public class TpACommand implements CommandExecutor {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        ...
        if(MapManager.isInAnyMaps(player) != null){
            player.sendMessage(Component.text("[ TPA ] ").color(TextColor.color(255, 255, 255)).append(Component.text("PvP장에서는 TPA 요청을 보낼 수 없습니다!").color(TextColor.color(255, 0, 0))));
            return false;
        }

        Player sendPlayer = Bukkit.getPlayer(args[0]);
        if(sendPlayer == null){
            player.sendMessage(Component.text("[ TPA ] ").color(TextColor.color(255, 255, 255)).append(Component.text(args[0] + "님은 오프라인입니다!").color(TextColor.color(255, 0, 0))));
            return false;
        }
        ...
        if(command.getLabel().equalsIgnoreCase("tpa") && !sendPlayer.getName().equals(player.getName())){
            ...
            player.sendMessage(Component.text("[ TPA ] ").color(TextColor.color(255, 255, 255))
                    .append(Component.text(args[0] + "님에게 TpA를 보냈습니다!").color(TextColor.color(0, 255, 0))));
            sendPlayer.sendMessage(Component.text("[ TPA ] ").color(TextColor.color(255, 255, 255))
                    .append(Component.text(player.getName() + "님께서 TpA를 보냈습니다!").color(TextColor.color(204, 153, 255)).append(
                            Component.text(" [ 수락하기 ]").color(TextColor.color(0, 255, 0)).clickEvent(ClickEvent.runCommand("/tpaccept " + player.getName())))));
